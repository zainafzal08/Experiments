...
class State {
    ...
    updateState(k,v) {
        this.state[k] = v;
        for(f of this.subscribers["stateUpdate"]) {
            e = {type: "stateUpdate", state}
            f(e);
        }
    }
    subscribe(e,f) {
        if (this.subscribers[e] === undefined) this.subscribers[e] = [];
        this.subscribers[e].push(f)
    }
}
...
function bindInput(e,v) {
    const type = e.attributes.type.value;
    if (type === "text") {
        e.addEventListener("change",(event)=>updateState(v,event.target.value));
    } else {
        throw Error(`Can not bind a ${type} Input Element`)
    }
}
...
function bind(e,v) {
    if (e.tagName === "INPUT") {
        ...
        throw Error(`Can not bind a ${e.tagName} Tag`);
    }
}

// stupid listener which grabs every update to check if it should change
// Not a huge deal this is more of a fun excersize with a small app
function conditionalDisplay(e,c) {
    subscribe("stateUpdate",(event)=>{
        if (eval(c)) {
            e.style.display = "";
            e.style.opacity = 1;
        } else {
...
function parseTree(root) {
    for(e of root.children) {
        if (e.attributes.bind !== undefined) bind(e,e.attributes.bind.value)
        if (e.attributes.if !== undefined) conditionalDisplay(e,e.attributes.if.value)
        parseTree(e);
    }
}
...
window.onload = function () {
    // parse the document to add in magic js
    parseTree(document.body);

    // add in a listener to attempt a feed render
    subscribe("stateUpdate",fetchFeed);
